 ...
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    if (size == out_dims[axis]) {
      if (reverse) {
        // T* modifier = const_cast<T*>(in_data);
        // thrust::device_ptr<T> dev_ptr(modifier);

        // thrust::device_ptr<T> dev_ptr(in_data);

        thrust::device_ptr<const T> dev_ptr =
            thrust::device_pointer_cast(in_data);
        thrust::device_vector<T> vec(dev_ptr, dev_ptr + size);
        if (exclusive) {
          thrust::exclusive_scan(vec.rbegin(), vec.rend(), out_data);
        } else {
          thrust::inclusive_scan(vec.rbegin(), vec.rend(), out_data);
        }
      } else {
        if (exclusive) {
